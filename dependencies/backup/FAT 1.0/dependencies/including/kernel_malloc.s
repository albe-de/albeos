...
kfree:
    ...
    # Get block header
    sub eax, 12
    mov ebx, eax                # ebx = current block

    # Mark as free
    mov dword ptr [ebx+4], 1

    # Try to coalesce with previous block
    mov esi, [free_list]        # esi = start of list
    xor edx, edx                # edx = previous block (NULL initially)
.prev_loop:
    test esi, esi
    jz .no_prev_merge
    cmp esi, ebx
...
.found_prev:
    test edx, edx
    jz .no_prev_merge           # no previous block
    cmp dword ptr [edx+4], 0    # is previous free?
    je .no_prev_merge

    # Merge current into previous
    mov eax, [ebx]              # current->size
    add eax, [edx]              # prev->size + current->size
    add eax, 12                 # + header size
    mov [edx], eax

    mov eax, [ebx+8]            # current->next
    mov [edx+8], eax            # prev->next = current->next

    mov ebx, edx                # now work on merged block
